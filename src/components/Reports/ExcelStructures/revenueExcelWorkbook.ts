...
export const generateRevenueExcelWorkbook = (reportData: RevenueReportData[]): XLSX.WorkBook => {
  ...
  const revenueSummaryData = reportData.map(warehouse => ({
    ...
    'Revenue per Customer': warehouse.customerDetails.length > 0 
      ? formatCurrency(warehouse.totalRevenue / warehouse.customerDetails.length) 
      : 0,
  }));
  XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(revenueSummaryData), 'Revenue Summary');
  ...
  reportData.forEach(warehouse => {
    ...
    const totalCustomerRevenue = warehouse.customerDetails.reduce((sum, customer) => sum + customer.amount, 0);
    const averageRevenuePerCustomer = warehouse.customerDetails.length > 0 
    ...
    warehouseDetailsData.push({
      ...
      'Total Expenses': formatCurrency(warehouse.totalExpenses),
      'Net Revenue': formatCurrency(warehouse.netRevenue),
      'Profit Margin (%)': warehouse.totalRevenue > 0 
      ...
      'Revenue vs Customer Revenue Diff': formatCurrency(warehouse.totalRevenue - totalCustomerRevenue),
    });
  });
  XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(warehouseDetailsData), 'Warehouse Details');
  ...
  reportData.forEach(warehouse => {
    warehouse.customerDetails.forEach(customer => {
      customerRevenueData.push({
        ...
        'Percentage of Warehouse Revenue': warehouse.totalRevenue > 0 
          ? ((customer.amount / warehouse.totalRevenue) * 100).toFixed(2) 
          : 0,
      });
    });
  });
  XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(customerRevenueData), 'Customer Revenue');

  // Overall Analytics Sheet
  const overallAnalytics = [];
  
  // Calculate overall totals
  const totalRevenue = reportData.reduce((sum, warehouse) => sum + warehouse.totalRevenue, 0);
  const totalExpenses = reportData.reduce((sum, warehouse) => sum + warehouse.totalExpenses, 0);
  const totalNetRevenue = reportData.reduce((sum, warehouse) => sum + warehouse.netRevenue, 0);
  const totalCustomers = reportData.reduce((sum, warehouse) => sum + warehouse.customerDetails.length, 0);
  ...
  overallAnalytics.push({
    ...
    'Value': totalRevenue > 0 ? ((totalNetRevenue / totalRevenue) * 100).toFixed(2) : 0,
    'Type': 'Percentage'
  });
  overallAnalytics.push({
  ...
  overallAnalytics.push({
    ...
    'Value': totalWarehouses > 0 ? formatCurrency(totalRevenue / totalWarehouses) : 0,
    'Type': 'Currency'
  });
  overallAnalytics.push({
    'Metric': 'Average Revenue per Customer',
    'Value': totalCustomers > 0 ? formatCurrency(totalRevenue / totalCustomers) : 0,
    'Type': 'Currency'
  });
  overallAnalytics.push({
    'Metric': 'Average Customers per Warehouse',
    'Value': totalWarehouses > 0 ? (totalCustomers / totalWarehouses).toFixed(2) : 0,
    'Type': 'Decimal'
  });
  ...
  const warehousesByRevenue = [...reportData].sort((a, b) => b.totalRevenue - a.totalRevenue);
  ...
  const warehousesByNetRevenue = [...reportData].sort((a, b) => b.netRevenue - a.netRevenue);
  ...
  XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(topPerformersData), 'Top Performers');
