...
export const generateUnitExcelWorkbook = (
  ...
  const unitSummaryData = reportData.map((unit) => {
    ...
    const cancelledBookings = unit.bookings.filter(
    ...
    const pendingPayments = unit.bookings.reduce(
      (sum, booking) =>
        sum + booking.payments.filter((p) => p.status === 'pending').length,
      0,
    );
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(unitSummaryData),
    'Unit Summary',
  );
  ...
  reportData.forEach((unit) => {
    unit.bookings.forEach((booking) => {
      ...
      bookingDetailsData.push({
        ...
        'Paid Payments': booking.payments.filter((p) => p.status === 'paid')
          .length,
        'Pending Payments': booking.payments.filter(
          (p) => p.status === 'pending',
        ).length,
  ...
  const unitUtilizationData = reportData.map((unit) => {
    ...
    const utilizationRate =
      totalBookings > 0
        ? (
            ((activeBookings + completedBookings) / totalBookings) *
            100
          ).toFixed(2)
        : '0';
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(unitUtilizationData),
    'Unit Utilization',
  );
