...
export const generateOccupancyExcelWorkbook = (
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(occupancySummaryData),
    'Occupancy Summary',
  );
  ...
  occupancyData.forEach((report) => {
    ...
    report.occupiedUnitDetails.forEach((unit) => {
      const totalBookingRevenue = unit.bookings.reduce(
      ...
      const totalPaidAmount = unit.bookings.reduce((sum, booking) => {
        return (
          sum +
          booking.payments.reduce((paySum, payment) => {
            return (
              paySum +
      ...
      unitDetailsData.push({
        ...
        'Outstanding Amount': formatCurrency(
          totalBookingRevenue - totalPaidAmount,
        ),
        'Revenue per Sq Ft':
          unit.size > 0 ? formatCurrency(totalBookingRevenue / unit.size) : 0,
      });
    });
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(unitDetailsData),
    'Unit Details',
  );
  ...
  occupancyData.forEach((report) => {
    report.occupiedUnitDetails.forEach((unit) => {
      unit.bookings.forEach((booking) => {
        ...
        const failedPayments = booking.payments.filter(
        ...
        bookingDetailsData.push({
          ...
          'Outstanding Amount': formatCurrency(
            parseFloat(booking.price) - totalPayments,
          ),
          'Days Booked': Math.ceil(
            (new Date(booking.endDate).getTime() -
              new Date(booking.startDate).getTime()) /
              (1000 * 60 * 60 * 24),
          ),
          'Price per Day': formatCurrency(
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(bookingDetailsData),
    'Booking Details',
  );
      ...
      unit.bookings.forEach((booking) => {
        booking.payments.forEach((payment) => {
          paymentDetailsData.push({
            ...
            'Period Days': Math.ceil(
              (new Date(payment.endDate).getTime() -
                new Date(payment.startDate).getTime()) /
                (1000 * 60 * 60 * 24),
            ),
            'Amount per Day': formatCurrency(
              parseFloat(payment.amount) /
                Math.ceil(
                  (new Date(payment.endDate).getTime() -
                    new Date(payment.startDate).getTime()) /
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(paymentDetailsData),
    'Payment Details',
  );
  ...
  occupancyData.forEach((report) => {
    [...report.occupiedUnitDetails, ...report.availableUnitDetails].forEach(
      (unit) => {
  ...
  warehouseStats.forEach((stats, warehouseName) => {
    stats.averageUnitSize =
      stats.totalUnits > 0 ? stats.totalSize / stats.totalUnits : 0;
    const occupancyRate =
      stats.totalUnits > 0 ? (stats.occupiedUnits / stats.totalUnits) * 100 : 0;
    const revenuePerUnit =
      stats.totalUnits > 0 ? stats.totalRevenue / stats.totalUnits : 0;
    const revenuePerSqFt =
      stats.totalSize > 0 ? stats.totalRevenue / stats.totalSize : 0;
    ...
    warehouseAnalysisData.push({
      ...
      'Average Bookings per Occupied Unit':
        stats.occupiedUnits > 0
          ? (stats.totalBookings / stats.occupiedUnits).toFixed(2)
          : 0,
    });
  });
  ...
  warehouseAnalysisData.sort(
    (a, b) =>
      parseFloat(b['Occupancy Rate (%)']) - parseFloat(a['Occupancy Rate (%)']),
  );
  XLSX.utils.book_append_sheet(
    workbook,
  ...
  customerStats.forEach((stats, customerName) => {
    customerAnalysisData.push({
      ...
      'Average Revenue per Booking': formatCurrency(
        stats.totalRevenue / stats.totalBookings,
      ),
      'Average Space per Booking (sq ft)': (
        stats.totalSpaceOccupied / stats.totalBookings
      ).toFixed(2),
    });
  });

  customerAnalysisData.sort(
    (a, b) => parseFloat(b['Total Revenue']) - parseFloat(a['Total Revenue']),
  );
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(customerAnalysisData),
    'Customer Analysis',
  );
