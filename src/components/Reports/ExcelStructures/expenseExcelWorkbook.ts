...
export const generateExpenseExcelWorkbook = (
  ...
  const expenseSummaryData = expenseData.map((expense) => ({
    ...
    Month: new Date(expense.date).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
    }),
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(expenseSummaryData),
    'Expense Summary',
  );

  // Warehouse Analysis Sheet
  const warehouseAnalysisData: any[] = [];
  const warehouseGroups = expenseData.reduce(
    (acc, expense) => {
    ...
    {} as Record<
      number,
      { warehouseName: string; expenses: ExpenseReportData[] }
    >,
  );
  ...
  Object.entries(warehouseGroups).forEach(([warehouseId, data]) => {
    ...
    const dateRange = {
      earliest: new Date(
        Math.min(...data.expenses.map((e) => new Date(e.date).getTime())),
      ),
      latest: new Date(
        Math.max(...data.expenses.map((e) => new Date(e.date).getTime())),
      ),
    };
    ...
    warehouseAnalysisData.push({
      ...
      'Highest Single Expense': formatCurrency(
        Math.max(...data.expenses.map((e) => e.total)),
      ),
      'Lowest Single Expense': formatCurrency(
        Math.min(...data.expenses.map((e) => e.total)),
      ),
    });
  });

  // Sort by total expenses descending
  warehouseAnalysisData.sort(
    (a, b) => b['Total Expenses'] - a['Total Expenses'],
  );
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(warehouseAnalysisData),
    'Warehouse Analysis',
  );

  // Expense Type Analysis Sheet
  const expenseTypeAnalysisData: any[] = [];
  const expenseTypeGroups = expenseData.reduce(
    (acc, expense) => {
  ...
  Object.entries(expenseTypeGroups).forEach(([expenseType, expenses]) => {
    ...
    const percentageOfTotal =
      totalExpenseAmount > 0 ? (totalAmount / totalExpenseAmount) * 100 : 0;
    ...
    expenseTypeAnalysisData.push({
      ...
      'Highest Amount': formatCurrency(
        Math.max(...expenses.map((e) => e.total)),
      ),
      'Lowest Amount': formatCurrency(
        Math.min(...expenses.map((e) => e.total)),
      ),
      'Most Frequent Warehouse': expenses.reduce(
        (acc, expense) => {
  ...
  expenseTypeAnalysisData.forEach((item) => {
    ...
    const mostFrequent = Object.entries(warehouseFreq).sort(
      ([, a], [, b]) => (b as number) - (a as number),
    )[0];
    item['Most Frequent Warehouse'] = mostFrequent
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(expenseTypeAnalysisData),
    'Expense Type Analysis',
  );

  // Monthly Trends Sheet
  const monthlyTrendsData: any[] = [];
  const monthlyGroups = expenseData.reduce(
    (acc, expense) => {
  ...
  Object.entries(monthlyGroups).forEach(([month, expenses]) => {
    ...
    const expenseCount = expenses.length;
    const averageAmount = expenseCount > 0 ? totalAmount / expenseCount : 0;
    const uniqueWarehouses = [...new Set(expenses.map((e) => e.warehouseId))]
      .length;
    const uniqueExpenseTypes = [...new Set(expenses.map((e) => e.expenseType))]
      .length;
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(monthlyTrendsData),
    'Monthly Trends',
  );
  ...
  const totalWarehouses = [...new Set(expenseData.map((e) => e.warehouseId))]
    .length;
  const totalExpenseTypes = [...new Set(expenseData.map((e) => e.expenseType))]
    .length;
  const averageExpenseAmount =
    totalEntries > 0 ? totalExpenses / totalEntries : 0;
  const averageExpensesPerWarehouse =
    totalWarehouses > 0 ? totalExpenses / totalWarehouses : 0;
  const averageEntriesPerWarehouse =
    totalWarehouses > 0 ? totalEntries / totalWarehouses : 0;
  ...
  sortedExpenses.forEach((expense, index) => {
    topExpensesData.push({
      ...
      'Percentage of Total':
        totalExpenses > 0
          ? ((expense.total / totalExpenses) * 100).toFixed(2)
          : 0,
    });
  });
  ...
  XLSX.utils.book_append_sheet(
    workbook,
    XLSX.utils.json_to_sheet(topExpensesData),
    'Top Expenses',
  );
